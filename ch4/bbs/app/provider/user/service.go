...
func (u *UserService) Register(ctx context.Context, user *User) (*User, error) {
	...
	userDB := &User{}
	if db.Where(&User{Email: user.Email}).First(userDB).Error.Error() != gorm.ErrRecordNotFound.Error() {
		return nil, errors.New("邮箱已注册用户，不能重复注册")
	}
	// 判断用户名已经注册
	if db.Where(&User{UserName: user.UserName}).First(userDB).Error.Error() != gorm.ErrRecordNotFound.Error() {
		return nil, errors.New("用户名已注册用户，不能重复注册")
	}
	token := genToken(16)
	// 生成token
	user.Token = token
	...
	if err := cacheService.SetObj(ctx, key, user, time.Hour * 24);err != nil {
		return nil, err
	}
	return user, nil
}
...
func (u *UserService)SendRegisterMail(ctx context.Context, user *User) error {
	...
	if err := d.DialAndSend(m); err != nil {
		logger.Error(ctx, "send email error", map[string]interface{}{ "err": err, "message": m, })
		return err
	}
	return nil
}
...
func (u *UserService)Login(ctx context.Context, user *User) (*User, error) {
	...
	// 根据名称查询用户
	userDB := &User{}
	if db.Where("username=?", user.UserName).First(userDB).Error != nil {
...
func (u *UserService)Logout(ctx context.Context, user *User) error {
	...
	if err := cacheService.Del(ctx, key);err != nil {
		return err
	}
	return nil
}
